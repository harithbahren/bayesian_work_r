...
# Chapter15
n.trials <- 100000
prior.alpha <- 700 #strong prior based on prob of clicking the link to the blog being 0.3 and applied to both A and B
prior.beta <- 300  
a.samples <- rbeta(n.trials,36+prior.alpha,114+prior.beta)
b.samples <- rbeta(n.trials,50+prior.alpha,100+prior.beta)
p.b_superior <- sum(b.samples > a.samples)/n.trials #how many times b.samples is greater than a.samples
p.b_superior
#Variant B is superior 73% of the time from 96%
n.trials <- 100000
a.prior.alpha <- 30 #weaker prior based on the prob of clicking the link to the blog being 0.3
a.prior.beta <- 70 
b.prior.alpha <- 20 #medium prior based on lead designer's advice that the prob of clicking the link for B (no images) is 0.2
b.prior.beta <- 80
